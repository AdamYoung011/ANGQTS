...
{
	ofstream output("./EWFA_2020-202106/fund/short/train/H2H/fund_" + str);
...
{
	ofstream output("./EWFA_2020-202106/short/train/H2H_train.csv", ios_base::app);
	output << fixed << setprecision(20);
	output << str << ",";
...
{
	ofstream output("./EWFA_2020-202106/fund/short/test/H2H/fund_" + str);
	output << fixed << setprecision(20);
...
{
	ofstream output("./EWFA_2020-202106/short/test/H2H_test.csv", ios_base::app);
	output << fixed << setprecision(20);
	output << str << ",";
...
int main()
{
	...
	for (auto& p : fs::directory_iterator("./2020-202106/H2H/train"))
	{
		train_file.push_back(p.path());
	}
	for (auto& p : fs::directory_iterator("./2020-202106/H2H/test"))
	{
	...
	else
	{
		ofstream output("PF.csv", ios_base::app);
		output << fixed << setprecision(20);
		output << "H2H," << positive_return / negative_return << endl;
		output.close();
	}
	ofstream output("MDD_AVE.csv", ios_base::app);
	output << fixed << setprecision(20);
	output << "H2H," << mdd_sum / test_file.size() << endl;
